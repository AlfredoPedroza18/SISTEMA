					...
					<div class="vertical-box">
						...
						<div class="vertical-box-column width-250">
							...
							<div class="wrapper">
								...
								<p></p>
								<hr class="hidden-sm hidden-xs">
								<!--p class="hidden-sm hidden-xs"><b>Lista de archivos</b></p>
								<div class="table-responsive hidden-sm hidden-xs" style="height: 300px;">
									<table class="table table-valign-middle m-b-0 table-striped">
								<thead> 
									<tr>	
							...
							<div class="wrapper">
								<div class="p-30 bg-white">
									<!-- begin email form -->
										...
										<div class="m-b-15">
											<input type="text" class="form-control" id="asunto-email" />
										</div>
										<!-- end email subject -->
									...
									<form action="#" class="form-inline" method="POST" enctype="multipart/form-data" accept-charset="UTF-8" id="frm-upload-file-email">		
											
										<input type="hidden" name="_token" value="{{ csrf_token() }}">
										<div class="form-group">
										    <input type="file" class="form-control" name="file_email" id="file-upload">
										 </div>
										 
										  <button type="submit" class="btn btn-primary btn-sm" id="btn-upload-file_email">Subir archivo</button>
										<div class="m-b-15 col" id="search-mail-contact">
											
										</div>
										<div id="files-send-email">
										...
										<div class="m-b-15">
											<textarea class="textarea form-control" id="contenido-email" placeholder="Ingrese algún texto ..." rows="12"></textarea>
										</div>
...
<script>
	$(document).ready(function() {
		...
		$("#email-to-mio").tagit({
            ...
            afterTagAdded:function(event, ui){
    			...
    			clase_mv  	 = clase.replace('.','-');
    			clase_final  = clase_mv.replace('.','-'); 
            	
            	str_html= '<tr class="row-remove-email '+clase_final+ '"><td><label class="label label-primary">'+ui.tagLabel+ ' <span class="fa fa-times remove-email" ></span></label><input type="hidden" class="lista-nombre-correos" value="'+ui.tagLabel+'"></td></tr>';
		...
		$('#btn-upload-file_email').click(function(event){
			event.preventDefault();
			datos = $('#file-upload')[0].files;
	...
	var uploadFileEmail = function (){
		
		var datos_file 		  = new FormData();
		var token 	   		  = $('meta[name="csrf-token"]').attr('content');
		datos_file.append('file_email',$('#file-upload')[0].files);		
	    $.ajax({
	    	headers: {'X-CSRF-TOKEN':token},
		    url: '{{ url('upload_file_email') }}',
		    data: new FormData($("#frm-upload-file-email")[0]),
		    dataType:'json',
		    async:false,
		    type:'post',
		    processData: false,
		    contentType: false,
		    success:function(response){
		    	$('#files-send-email').append('<div class="contenedor-file" style="display:none;"><input type="hidden" class="name-file" name="file-upload-email" value="'+response+'"></div>');
		    	$('#files-send-email').append('<label class="label label-success descripcion-file">'+response+ '&nbsp;&nbsp;<span class="fa fa-times remove-file" ></span></label>&nbsp;&nbsp;');
	...
	var removerfile = function(){
		$('.remove-file').click(function(){
    		var indice 	  = $('.remove-file').index(this);
    		var elemento  = $('.descripcion-file').get(indice);
    		var name_file = $('.contenedor-file').get(indice);
	...
	var enviarMail = function(){
		...
		var token 	        	= $('meta[name="csrf-token"]').attr('content');
		var files_email			= [];
		var cantidad_archvios 	= $('.name-file').length;

		for(i=0; i< cantidad_archvios; i++){
			obj_file 	= $('.name-file').get(i);
			valor_file  = obj_file.value;
			files_email.push(valor_file);
		}		
		...
		if(longitud == 0){
			...
			//Genera el string de los emails
				...
				$.ajax({
		            ...
		            type:'GET',
		            dataType: 'json',
		            data: {contenido_email:contenido_email,para:str_emails,asunto:asunto,files:files_email},
		            success: function(response){ 
		            		
		                    if(response.status_alta=='success' || response.status_alta=='wrong'){
		                        if(response.status_alta == 'success'){
		                            swal({                                  
		                                    title: "<h3>¡ El correo se envío con éxito !</h3> ",
		                                    html: true,
		                                   // data: datos,
		                                    type: "success",   
	...
	var limpiar_datos = function(){
		...
		$('#files-send-email').empty();
		$('#email-to-mio').tagit('removeAll');
		$("#asunto-email").val('');
		...
		swal({   
					...
					confirmButtonColor: 'ef9d1e'
						...
						setTimeout(function(){  
							...
							$('.'+clase_final).fadeOut(500,function(){
								$('.'+clase_final).remove();							
							});
