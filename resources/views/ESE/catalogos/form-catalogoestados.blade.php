...
<li><a href="{{ route('sig-erp-ese::catalogosese.index') }}">Catálogos ESE</a></li>
		<li><a href="{{route('sig-erp-ese::CatalogoEstados.index')}}">Estados</a></li>
		<li>Alta Estados</li>
   </ol>
<h1 class="page-header text-center">Estados <small>Alta</small></h1>
	...
	<div class="panel panel-inverse" data-sortable-id="ui-widget-14">
                        <div class="panel-heading">
                            ...
                            <h4 class="panel-title">Estados <small>Alta</small></h4>
                        </div>
                        <div class="panel-body">
                        <form id="form-alta-estados">
                       
                          @include('ESE.catalogos.estadoscreate')
                        </form>
   </div>
 </div>
@endsection
...
$("#form-alta-estados").on("submit",function(event){
...
//------------------------- END DIRECCION FISCAL -------------------------------------------------------------------
    ...
    <script>
    function hacer_click_Edo(valor){


    var datos = $("#form-alta-estados").serialize();
    var token = $('meta[name="csrf-token"]').attr('content');
    console.log(datos);
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if(response.status_alta == 'success'){
                    ...
                    setTimeout(function(){
                        location.href = '{{ route("sig-erp-ese::CatalogoEstados.index") }}';
                    });
                }else if(response.status_alta == 'wrong'){
                    swal({
                        title: "<h3>¡ El registro NO se guardo con éxito !</h3> ",
