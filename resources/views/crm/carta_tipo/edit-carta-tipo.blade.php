...
<div id="content" class="content">
	...
	<ol class="breadcrumb ">
		...
		<li><a href="{{ route('sig-erp-crm::cartas_tipo.index') }}">Listado Cartas Tipo</a></li>
		<li>Edición Carta Tipo</li>

	</ol>
	
	<h1 class="page-header text-center">ALTA CARTA TIPO <small>EDITAR</small></h1>
	...
	<div class="panel panel-inverse" data-sortable-id="ui-widget-14" data-init="true">
                        ...
                        <div class="panel-body">               	
	                        		...
	                        		<input type="hidden" name="id_ct" id="id_ct" value="{{ $carta_tipo->id }}">            		
									<div>
										<div>
											<label class="control-label">Titulo:</label>
											<div class="m-b-15">
												<input type="text" name="titulo" id="titulo" class="form-control" id="asunto-email" value="{{ $carta_tipo->titulo }}" />
											</div>
											<!-- end email to -->
											<!-- begin email subject -->
											<label class="control-label">Descripción:</label>
											<div class="m-b-15">
												<input type="text" name="descripcion" id="descripcion"  class="form-control" value="{{ $carta_tipo->descripcion }}"/>
											</div>
											...
											<textarea id="elm1" rows="15" cols="80" style="width: 80%" "{!! $carta_tipo->contenido !!}">
	...
	<script>
		$(document).ready(function(){
			...
			$('#btn-enviar').click(function(event){
				...
				actualizarCartaTipo();
				setTimeout(function(){
				    location.reload('{{ route('sig-erp-crm::cartas_tipo.index') }}');
				}, 1000);
		...
		var actualizarCartaTipo = function(){
	        ...
	        $.ajax({
	            ...
	            url:'{{ route('sig-erp-crm::cartas_tipo.store').'/' }}'+id_ct,
	            type:'PUT',
	            dataType: 'json',
	            ...
	            success: function(response){ 
	                    ...
	                    if(response.status_alta=='success' || response.status_alta=='wrong'){
	                        if(response.status_alta == 'success'){
	                            ...
	                            setTimeout(function(){
	                            	location.href = '{{ route('sig-erp-crm::cartas_tipo.index') }}';
	                            },2000);
